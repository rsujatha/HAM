...
for i in NofPoints:
	...
	for n in range (X[:i].size):			#Put X.size later
		hist_temp,edge = pcf.counter(X[:i],Y[:i],Z[:i],n,True,bins) 			 
		hist+= hist_temp
	t=time.time()-t
	T3.append(t)
	...
	##brute force
	t=time.time()
	hist=pcf.counter_bruteforce(X[:i],Y[:i],Z[:i],0.1)
	t=time.time()-t
	T1.append(t)
	
	#~ ##kdtree
	t=time.time()
	hist,edge=pcf.counter_kdtree(X[:i],Y[:i],Z[:i],0.1)
	t=time.time()-t
	T2.append(t)
	...
	t=time.time()
	hist,edge=pcf.counter_spherical_select(X[:i],Y[:i],Z[:i],0.1)
	t=time.time()-t
	T4.append(t)
	
	
	
np.savetxt('Time1.txt',np.transpose([NofPoints,T1,T2,T3,T4]), header = 'No of Points \t bruteforce \t kdtree \t cubical \t spherical')	
