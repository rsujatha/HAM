...
yang_data = np.loadtxt("yangDR7_centrals.txt",skiprows=1,usecols=[1],unpack=True)
...
np.savetxt('x_y_z_m_sim.txt',np.transpose([X,Y,Z,M200b,PID]),fmt=['%.6f','%.6f','%.6f','%.6f','%.6f'], header = 'X\tY\tZ\tM200b\tPID')
